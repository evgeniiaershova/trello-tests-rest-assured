...
import dataloaders.http.annotations.QueryParameter;
import dataloaders.http.response.ResponseStatusType;
import io.restassured.http.ContentType;
import io.restassured.http.Header;
import io.restassured.specification.RequestSpecification;

import static dataloaders.http.response.ResponseStatusType.OK;
...
public class RestMethod<T> {
    ...
    public RestMethod(RestMethodTypes type, String url, String resource) {
        spec.baseUri(url);
...
//        data.headers.add(name, value);
    ...
    public void addHeader(dataloaders.http.annotations.Header header) {
        addHeader(header.name(), header.value());
    }
    public void setQP(String name, String value) {
        spec.queryParams(name, value);
    }

    public void addHeader(Header header) {
        addHeader(header.getName(), header.getValue());
    }
    public void addHeaders(dataloaders.http.annotations.Header... headers) {
        for(dataloaders.http.annotations.Header header : headers)
            addHeader(header);
    }
...
//        data.contentType = ct;
    ...
    public void addHeaders(Header[] headers) {
     ...
     void addQueryParameters1(QueryParameter... params) {
        ...
        for(QueryParameter queryParameter: params) {
            spec.queryParams(queryParameter.name(), queryParameter.value());
        }
    ...
    void addQueryParameters(QueryParameter... params) {
        ...
        queryParams.addAll(new MapArray<>(params,
                QueryParameter::name, QueryParameter::value));
        if (queryParams.any()) {
            spec.queryParams(queryParams.toMap());
        }
    }
